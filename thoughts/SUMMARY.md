...
Index of the project's working documents. Each summary lives in its own file; this page only says
where to look.

- `specs/` — product requirements, one per feature, amended in place with dated sections.
- `plans/` — phased implementation plans; the `Status` line names the release tag that shipped
  them.
- `tests/` — test manifests (Tests-as-DoD): traceability from spec item to test file.
