...
**Status**: all passing (40 executable, 5 live-only todos, 6 Swift) — amended 2026-09-01 and 2026-09-02 (see the amendment notes at the end)

---

## Framework and Conventions

- TypeScript: Vitest, `describe`/`it`, `expect`, static imports of the product modules (since
  2026-09-02), boundary fakes, isolated temporary homes, and top-level `test/*.test.ts` files.
...
- Omarchy: Vitest data assertions over the pinned manifest and helper modes; helper behaviour in
  `test/omarchy-bootstrap.test.ts` and `test/omarchy-connections.test.ts`; negative source checks in
  `test/source-contract.test.ts` (source contract, not acceptance).
...
exist. Since 2026-09-02 they import the product modules statically and type-check against them.
Every executable case passes; the five `todo` cases remain live-only release gates.
...
| US-2/AC-2 | Partial success remains visible and recoverable       | setup      | Focused connector retry (Agents card copy grep removed 2026-09-02, no behaviour) | Passing |
...
| FR-1.3    | OS-managed daemon independent from UI                | runtime              | Rendered LaunchAgent and unit parsed                                                     | Passing             |
...
| FR-2.10   | Common domain-neutral connector contract             | connectors           | Registry parity; QML boundary: source contract                                           | Passing             |
...
| FR-8.1    | Update reconciles installation transactionally       | lifecycle            | Packaged update transaction                                                              | Passing             |
...
| FR-9.1    | JSON-first, non-interactive, redacted CLI            | CLI                  | Connection command envelope; helper: `test/omarchy-connections.test.ts`, source contract | Passing             |

### Security and privacy

| Spec item | Description                                     | File                  | Test contract                                                                                           | Status              |
| --------- | ----------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------- | ------------------- |
| SEC-1     | Logged-in user only; never root                 | runtime               | User unit without `User=`; bootstrap: source contract                                                   | Passing             |
| SEC-2     | HTTP/MCP network listeners remain loopback      | `test/server.test.ts` | Loopback binding enforced at listen                                                                     | Passing             |
| SEC-3     | Private directory and `0600` token/state files  | setup/security        | Journal/config modes                                                                                    | Passing             |
| SEC-4     | Tokens absent from all observable surfaces      | all                   | Commands, output, config; helpers: source contract                                                      | Passing             |
| SEC-5     | Sign/notarize app and nested executable code    | runtime               | `test/macos-package-artifact.test.ts`; entitlements parse in `test/runtime-release-integration.test.ts` | Live todo           |
| SEC-6     | Runtime pinned to application release           | runtime/surfaces      | Versioned manifest, no latest                                                                           | Passing             |
| SEC-7     | Verify Omarchy/runtime artifact trust chain     | surfaces              | Pinned manifest; bootstrap behaviour in `test/omarchy-bootstrap.test.ts`                                | Passing + live todo |
...
| SEC-11    | No direct database access through connectors/UI | source contract       | `test/source-contract.test.ts`                                                                          | Passing             |
| SEC-12    | No telemetry                                    | source contract       | `test/source-contract.test.ts`                                                                          | Passing             |
...
| EC-14     | Quickshell restarts                         | runtime/surfaces | Parsed unit ownership; QML boundary: source contract; live gate | Passing + live todo |
...
| PERF-6    | Daemon survives app/QML/host shutdown           | runtime     | Parsed OS ownership; no UI stop: source contract                         | Passing   |
| A11Y-1    | Keyboard/focus/VoiceOver/motion/theme preserved | macOS       | Swift accessibility tests (QML grep removed 2026-09-02, no behaviour)    | Passing   |
...
| A11Y-4    | Progress does not rely on color                 | macOS       | Labels and accessibility metadata (QML grep removed 2026-09-02)          | Passing   |
| A11Y-5    | Shared vocabulary on macOS and Omarchy          | macOS       | `SetupModelsTests.swift` (Omarchy half removed 2026-09-02, no behaviour) | Passing   |
...
| ---------- | ------------------------------------------------------------------------------------ | ------: | ----: | ------------------------------------ |
| runtime    | `test/zero-friction-runtime.acceptance.test.ts`                                      |       6 |     3 | Runtime, packaging, service          |
...
| lifecycle  | `test/zero-friction-lifecycle-security.acceptance.test.ts`                           |       8 |     0 | Migration, update, removal, security |
| CLI        | `test/zero-friction-cli.acceptance.test.ts`                                          |       4 |     0 | JSON-first CLI                       |
| surfaces   | `test/zero-friction-surfaces.acceptance.test.ts`                                     |       2 |     2 | Pinned Omarchy assets                |
...
- Generated cases: **51** — 46 executable cases, all passing, and 5 explicit live-only todos.
- TypeScript cases: **45** — 40 passing and 5 todos.
...
- `npx vitest run test/zero-friction-*.test.ts --reporter=verbose`: 6 files, 40 passed, 5 todo.
...
Status is the observed result on 2026-09-02.

| File                                                                                 | Spec sections covered                                         | Status            |
| ------------------------------------------------------------------------------------ | ------------------------------------------------------------- | ----------------- |
| `test/zero-friction-runtime.acceptance.test.ts`                                      | US-1, US-3, FR-1, FR-5.3, SEC-1, SEC-6, SEC-7, PERF-1, PERF-6 | 6 passing, 3 todo |
...
| `test/zero-friction-lifecycle-security.acceptance.test.ts`                           | US-3, FR-5.2, FR-8, SEC-3, SEC-8, SEC-9, EC-5–EC-8            | 8 passing         |
| `test/zero-friction-cli.acceptance.test.ts`                                          | FR-4.2, FR-4.3, FR-7.2, FR-9.1, US-4/AC-4                     | 4 passing         |
| `test/zero-friction-surfaces.acceptance.test.ts`                                     | FR-1.1, SEC-6, SEC-7, SEC-8; todos SEC-7, EC-14               | 2 passing, 2 todo |
...
- The generated files are no longer frozen. Their `@immutable` headers were removed on 2026-09-02
  (Task 8.2). The `@generated-from` reference and the spec-id test titles remain
...
Source: `thoughts/reviews/2026-09-01_deep-review.md`, findings H-13 and H-14, and Tasks 8.1 and
8.2 of `thoughts/plans/2026-09-01_deep-review-remediation.md`.

- The five TypeScript files import the product modules statically; the dynamic imports with local
  `as unknown as …` casts are gone, so every fixture now type-checks against the real signatures.
- `surfaces` keeps the two data assertions (pinned `runtime-manifest.json`, executable bootstrap)
  and its two todos. Its greps over `build-macos-app.sh`, `check-macos-artifact.mjs`,
  `update.ts`, `UpdateSettingsStore.swift`, the bootstrap and connections helpers, the Agents card
  and `SetupModels.swift` were removed 2026-09-02, no behaviour; the rows above point at the Swift
  suites, `test/omarchy-bootstrap.test.ts`, `test/omarchy-connections.test.ts` and the source
  contract instead.
- `runtime` renders the LaunchAgent and the systemd unit and asserts on the parsed artefacts
  (FR-1.3, PERF-6); the FR-1.1/SEC-5 release-tooling grep was removed 2026-09-02, no behaviour.
- `lifecycle` dropped the SEC-1–SEC-4/SEC-10–SEC-12 source scan; the negative halves live in
  `test/source-contract.test.ts` (source contract), the loopback listener in `test/server.test.ts`.
- Negative checks in `test/source-contract.test.ts` are source contract, never acceptance
  coverage; the matrix names them as such.
